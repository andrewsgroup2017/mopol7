package com.groupandrews.app.webview;
...
import com.groupandrews.app.Constants;
...
public class WebViewHelper {
    ...
    public void setupWebView(AdvertisingIdClient.Info adId) {
        Log.d(TAG, "[idInfo : " +adId + "]");
        idInfo = adId;
        // accept cookies
        CookieManager.getInstance().setAcceptCookie(true);
        CookieManager.getInstance().setCookie("http://192.168.1.6:9090/", "asdf");
        // enable JS
    ...
    public void loadIntentUrl(String url) {
        if (!url.equals("") && url.contains(Constants.WEBAPP_HOST)) {
            String str = "xxx";
//            webView.loadUrl("javascript:xxx('"+str+"')");
            url += str;
            webView.loadUrl(url);
        } else {
            // Fallback
...
//            Toast.makeText(mContext, toast, Toast.LENGTH_SHORT).show();
            Log.w(TAG, "addMessage:onFailure" + idInfo);
            String s = "nogood";
            try{
                s = idInfo.getId();
                Log.w(TAG, "addMessage:onFailure" + s);
