...
class TlsSocketFactory extends SSLSocketFactory {
    ...
    TlsSocketFactory(Context context) {
        ...
        factory = SSLCertificateSocketFactory.getDefault(com.groupandrews.app.WebHelper.SOCKET_TIMEOUT, cache);
    ...
    public Socket createSocket(Socket socket, String host, int port, boolean autoClose) throws IOException {
        ...
        if (socket != null && socket instanceof SSLSocket) {
            if (com.groupandrews.app.Logger.DEBUG) { Log.d(TAG, "[Preparing TLS socket]"); }
            ...
            if (com.groupandrews.app.Logger.DEBUG) { Log.d(TAG, "[Setting SNI for host " + host + "]"); }
            try {
                Method setHostnameMethod = sslSocket.getClass().getMethod("setHostname", String.class);
                setHostnameMethod.invoke(sslSocket, host);
            } catch (Exception e) {
                if (com.groupandrews.app.Logger.DEBUG) { Log.d(TAG, "[Setting SNI failed: " + e.getMessage() + "]"); }
            }

            try {
                InetSocketAddress remoteAddress = new InetSocketAddress(host, port);
                sslSocket.connect(remoteAddress, com.groupandrews.app.WebHelper.SOCKET_TIMEOUT);
                sslSocket.setSoTimeout(com.groupandrews.app.WebHelper.SOCKET_TIMEOUT);
    ...
    private void verifyHostname(SSLSocket sslSocket, String host) throws IOException {
        ...
        if (com.groupandrews.app.Logger.DEBUG) { Log.d(TAG, "Connected to " + session.getPeerHost() + " using " + session.getProtocol() + " (" + session.getCipherSuite() + ")"); }
    }
