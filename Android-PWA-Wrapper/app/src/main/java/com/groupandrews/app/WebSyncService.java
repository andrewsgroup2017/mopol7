...
public class WebSyncService extends IntentService {
    ...
    public void onCreate() {
        super.onCreate();
        if (com.groupandrews.app.Logger.DEBUG) { Log.d(TAG, "[websync create]"); }
    ...
    protected void onHandleIntent(Intent intent) {
        if (com.groupandrews.app.Logger.DEBUG) { Log.d(TAG, "[websync start]"); }

        if (pi != null) {
            // cancel pending alarm
            if (com.groupandrews.app.Logger.DEBUG) { Log.d(TAG, "[websync cancel alarm]"); }
    ...
    private int getTrackId() {
        ...
        if (trackId == 0) {
            ...
            try {
                ...
                if (com.groupandrews.app.Logger.DEBUG) { Log.d(TAG, "[websync io exception: " + e + "]"); }
                // schedule retry
                handleError(e);
            } catch (WebAuthException e) {
                if (com.groupandrews.app.Logger.DEBUG) { Log.d(TAG, "[websync auth exception: " + e + "]"); }
                isAuthorized = false;
                try {
    ...
    public void onDestroy() {
        if (com.groupandrews.app.Logger.DEBUG) { Log.d(TAG, "[websync stop]"); }
        if (db != null) {
            db.close();
        }
        super.onDestroy();
